...
function getProviderRequestUrl(provider) {
  ...
  const baseUrl = provider.type === "openai" ? OPENAI_BASE_URL : normalizeProviderBaseUrl(provider.baseUrl);
...
function getProviderApiKeyEnv(provider) {
...
function isLocalHttpUrl(value) {
  return value.protocol === "http:" && (value.hostname === "localhost" || value.hostname === "127.0.0.1" || value.hostname === "::1" || value.hostname === "[::1]");
}

// src/audit.ts
async function createAuditReport(config, configPath) {
  return {
    configPath,
...
function findDisallowedProviderEndpoints(report, allowedEndpoints) {
  const allowed = new Set(allowedEndpoints.map(normalizeEndpoint));
  return report.providerEndpoints.filter((endpoint) => !allowed.has(normalizeEndpoint(endpoint.endpoint))).map((endpoint) => ({
...
function assertAllowedProviderEndpoints(report, allowedEndpoints) {
...
function renderEndpointViolations(violations) {
...
function collectProviderEndpoints(config) {
  const endpoints = [];
  if (config.provider) {
...
function toAuditEndpoint(provider, scope, test) {
  return {
    kind: "provider",
    scope,
    test,
...
async function collectDependency(root, name, direct, seen) {
...
function normalizeRepositoryUrl(value) {
  ...
  return value.replace(/^git\+/u, "").replace(/^git:/u, "https:").replace(/^github:/u, "https://github.com/").replace(/\.git$/u, "");
}
...
async function loadConfig(configPath = "aici.yml") {
  const resolvedPath = path2.resolve(configPath);
  ...
  return {
    ...
    rootDir: path2.dirname(resolvedPath)
  };
}
function validateConfigShape(value) {
  ...
  if (value.$schema !== void 0 && typeof value.$schema !== "string") {
  ...
  if (value.redact !== void 0 && !isStringArray(value.redact)) {
  ...
  for (const [index, test] of value.tests.entries()) {
    ...
    if (test.mockOutput !== void 0 && typeof test.mockOutput !== "string") {
      throw new Error(`Test "${test.name}" field \`mockOutput\` must be a string.`);
    }
    if (test.mockOutputFile !== void 0 && typeof test.mockOutputFile !== "string") {
      throw new Error(`Test "${test.name}" field \`mockOutputFile\` must be a string.`);
    }
    if (test.mockToolCalls !== void 0 && !Array.isArray(test.mockToolCalls)) {
...
function validateProvider(value, label) {
  ...
  if (value.apiKeyEnv !== void 0 && typeof value.apiKeyEnv !== "string") {
    throw new Error(`${label}.apiKeyEnv must be a string.`);
  }
  if (value.api !== void 0 && value.api !== "responses" && value.api !== "chat-completions" && value.api !== "messages") {
    throw new Error(`${label}.api must be "responses", "chat-completions", or "messages".`);
  }
  if (value.baseUrl !== void 0 && typeof value.baseUrl !== "string") {
    throw new Error(`${label}.baseUrl must be a string.`);
  }
  if (value.apiVersion !== void 0 && typeof value.apiVersion !== "string") {
  ...
  if ((value.type === "openai" || value.type === "anthropic") && value.baseUrl !== void 0) {
    throw new Error(`${label}.baseUrl is only allowed for openai-compatible providers.`);
  }
  if (value.type === "anthropic" && value.api !== void 0 && value.api !== "messages") {
    throw new Error(`${label}.api must be "messages" for anthropic providers.`);
  }
  return value;
}
function validateExpect(value, label) {
  if (!isRecord(value)) {
    throw new Error(`${label} must be an object.`);
  }
  if (value.toolCalls !== void 0 && !Array.isArray(value.toolCalls)) {
...
function validateToolCallArray(value, label) {
...
function validateToolCallExpectation(value, label) {
  if (!isRecord(value)) {
    throw new Error(`${label} must be an object.`);
  }
  ...
  if (value.index !== void 0 && (typeof value.index !== "number" || !Number.isInteger(value.index) || value.index < 0)) {
    throw new Error(`${label}.index must be a non-negative integer.`);
  }
  if (value.argumentsContains !== void 0 && !isStringArray(value.argumentsContains)) {
    throw new Error(`${label}.argumentsContains must be an array of strings.`);
  }
  if (value.argumentsJsonSchema !== void 0 && typeof value.argumentsJsonSchema !== "string") {
...
function validateTools(value, label) {
  ...
  for (const [index, tool] of value.entries()) {
    ...
    if (tool.parametersFile !== void 0 && typeof tool.parametersFile !== "string") {
      throw new Error(`${label}[${index}].parametersFile must be a string.`);
    }
    if (tool.parameters !== void 0 && !isRecord(tool.parameters)) {
      throw new Error(`${label}[${index}].parameters must be an object.`);
    }
    if (tool.strict !== void 0 && typeof tool.strict !== "boolean") {
      throw new Error(`${label}[${index}].strict must be a boolean.`);
    }
  }
}
function validateToolChoice(value, label) {
...
async function writeReports(result, reportDir = ".aici") {
  const resolvedDir = path3.resolve(reportDir);
  await mkdir(resolvedDir, { recursive: true });
  await writeFile(path3.join(resolvedDir, "aici-report.json"), `${JSON.stringify(result, null, 2)}
`);
  await writeFile(path3.join(resolvedDir, "aici-report.md"), renderMarkdown(result));
  await writeFile(path3.join(resolvedDir, "aici-report.html"), renderHtml(result));
}
function renderMarkdown(result) {
  ...
  for (const test of result.results) {
    ...
    lines.push(
      `| ${escapeTable(test.name)} | ${test.passed ? "PASS" : "FAIL"} | ${escapeTable(test.model ?? "-")} | ${escapeTable(renderToolCalls(test.toolCalls))} | ${test.latencyMs ?? "-"}ms | ${test.costUsd === void 0 ? "-" : `$${test.costUsd.toFixed(6)}`} | ${escapeTable(
...
function renderHtml(result) {
  ...
  const rows = result.results.map((test) => {
    ...
    return `<tr>
      ...
      <td>${test.costUsd === void 0 ? "-" : `$${test.costUsd.toFixed(6)}`}</td>
...
function summarizeCategories(result) {
...
function resolveConfigFile(rootDir, filePath, label) {
...
var MODEL_PRICING_USD_PER_1M_TOKENS = {
...
async function callProvider(provider, test, prompt, rootDir) {
...
async function callAnthropicMessages(provider, test, prompt, rootDir) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Provider request failed for "${test.name}" (${response.status}): ${body2}`);
  }
...
async function callOpenAiCompatible(provider, test, prompt, rootDir) {
  if (provider.type === "anthropic") {
    throw new Error(`Unsupported provider type: ${provider.type}`);
  }
  if (provider.type === "openai" && provider.baseUrl !== void 0) {
  ...
  const api = provider.api ?? (provider.type === "openai" ? "responses" : "chat-completions");
  const request = api === "responses" ? () => callResponsesApi(provider, test, prompt, apiKey, rootDir) : () => callChatCompletionsApi(provider, test, prompt, apiKey, rootDir);
  return withRetries(request, provider.retries ?? 1);
}
async function callResponsesApi(provider, test, prompt, apiKey, rootDir) {
  const startedAt = performance.now();
  const tools = await resolveTools(test, rootDir);
  const requestUrl = getProviderRequestUrl(provider);
  const response = await fetchProvider(provider, requestUrl, {
...
async function callChatCompletionsApi(provider, test, prompt, apiKey, rootDir) {
  const startedAt = performance.now();
  const tools = await resolveTools(test, rootDir);
  const requestUrl = getProviderRequestUrl(provider);
  const response = await fetchProvider(provider, requestUrl, {
...
async function fetchProvider(provider, url, init, timeoutMs = 3e4) {
  const expectedUrl = getProviderRequestUrl(provider);
  if (normalizeEndpoint2(url) !== normalizeEndpoint2(expectedUrl)) {
...
async function withRetries(request, retries) {
  let lastError;
...
function extractResponsesText(body) {
  const parts = body.output?.flatMap((item) => item.content ?? []).map((content) => content.text).filter((text) => typeof text === "string");
  return parts && parts.length > 0 ? parts.join("\n") : void 0;
}
function extractAnthropicText(body) {
  const parts = body.content?.filter((item) => item.type === "text").map((item) => item.text).filter((text) => typeof text === "string");
  return parts && parts.length > 0 ? parts.join("\n") : void 0;
}
function extractAnthropicToolCalls(body) {
  const normalized = [];
...
function extractResponsesToolCalls(body) {
  const normalized = [];
...
async function resolveTools(test, rootDir) {
  const tools = [];
...
async function resolveToolParameters(tool, rootDir) {
  if (typeof tool.parametersFile === "string") {
    return JSON.parse(await readFile3(resolveConfigFile(rootDir, tool.parametersFile, `Tool "${tool.name}" parametersFile`), "utf8"));
  }
...
// src/checks.ts
async function runTest(test, rootDir, defaultProvider) {
...
async function executeTest(test, rootDir, defaultProvider) {
  const startedAt = performance.now();
  ...
  if (typeof test.mockOutputFile === "string") {
    return {
      output: await readFile4(resolveConfigFile(rootDir, test.mockOutputFile, `Test "${test.name}" mockOutputFile`), "utf8"),
      toolCalls: await resolveMockToolCalls(test, rootDir),
      latencyMs: Math.round(performance.now() - startedAt)
    };
  }
  if (test.mockToolCalls !== void 0 || typeof test.mockToolCallsFile === "string") {
    return {
      output: "",
      toolCalls: await resolveMockToolCalls(test, rootDir),
      latencyMs: Math.round(performance.now() - startedAt)
...
async function resolveMockToolCalls(test, rootDir) {
  if (test.mockToolCalls !== void 0) {
    return test.mockToolCalls;
  }
  if (typeof test.mockToolCallsFile === "string") {
    const raw = await readFile4(resolveConfigFile(rootDir, test.mockToolCallsFile, `Test "${test.name}" mockToolCallsFile`), "utf8");
...
async function resolvePrompt(test, rootDir) {
  const promptParts = [];
  if (typeof test.prompt === "string") {
    promptParts.push(test.prompt);
  }
  if (typeof test.promptFile === "string") {
    promptParts.push(await readFile4(resolveConfigFile(rootDir, test.promptFile, `Test "${test.name}" promptFile`), "utf8"));
  }
  if (typeof test.input === "string") {
    promptParts.push(test.input);
  }
  if (typeof test.inputFile === "string") {
    promptParts.push(await readFile4(resolveConfigFile(rootDir, test.inputFile, `Test "${test.name}" inputFile`), "utf8"));
  }
...
async function runChecks(expect, output, rootDir, usage) {
  const checks = [];
  if (expect.exact !== void 0) {
    checks.push({
      name: "exact",
  ...
  if (expect.json || expect.jsonSchema) {
    ...
    if (parsed.ok && expect.jsonSchema) {
      ...
      const schema = JSON.parse(await readFile4(schemaPath, "utf8"));
      ...
      checks.push({
        name: "jsonSchema",
        passed: valid,
        message: valid ? `Output matches ${expect.jsonSchema}.` : ajv.errorsText(validate.errors, { separator: "; " })
      });
    }
  }
  ...
  if (expect.maxLatencyMs !== void 0) {
    const latencyMs = usage.latencyMs;
    checks.push({
      name: "maxLatencyMs",
      passed: latencyMs !== void 0 && latencyMs <= expect.maxLatencyMs,
      message: latencyMs === void 0 ? "Latency is unknown." : `${latencyMs}ms <= ${expect.maxLatencyMs}ms.`
    });
  }
  if (expect.maxCostUsd !== void 0) {
    const costUsd = usage.costUsd;
    checks.push({
      name: "maxCostUsd",
      passed: costUsd !== void 0 && costUsd <= expect.maxCostUsd,
      message: costUsd === void 0 ? "Cost is unknown for this provider/model." : `$${costUsd.toFixed(6)} <= $${expect.maxCostUsd.toFixed(6)}.`
    });
  }
  if (checks.length === 0) {
    checks.push({
...
async function checkToolCall(expectation, toolCalls, rootDir, expectationIndex) {
  const checks = [];
  ...
  if (expectation.name !== void 0) {
    checks.push({
      name: "toolCall.name",
      passed: selected.name === expectation.name,
      message: selected.name === expectation.name ? `Called ${expectation.name}.` : `Expected ${expectation.name}, got ${selected.name}.`
    });
  }
  ...
  for (const expectedText of expectation.argumentsContains ?? []) {
    checks.push({
      ...
      message: argumentText.includes(expectedText) ? `Tool arguments contain "${expectedText}".` : `Tool arguments do not contain "${expectedText}".`
    });
  }
  if (expectation.argumentsJsonSchema) {
    ...
    checks.push({
      ...
      passed: valid,
      message: valid ? `Tool arguments match ${expectation.argumentsJsonSchema}.` : ajv.errorsText(validate.errors, { separator: "; " })
    });
  }
...
function selectToolCall(expectation, toolCalls) {
  if (expectation.index !== void 0) {
...
function collectRedactionValues(config) {
...
function redactTestResult(result, redactions) {
  return {
    ...result,
...
function redactText(value, redactions) {
  let result = value.replaceAll(/Bearer\s+[A-Za-z0-9._~+/=-]+/g, "Bearer [REDACTED]").replaceAll(/\b(?:sk|rk|pk|org|proj)-[A-Za-z0-9_-]{12,}\b/g, "[REDACTED]");
  for (const secret of redactions) {
    result = result.replaceAll(secret, "[REDACTED]");
  }
  return result;
}
function collectProviderEnvNames(config) {
...
function addProviderEnvName(envNames, provider) {
  if (provider?.apiKeyEnv) {
    envNames.add(provider.apiKeyEnv);
  }
}
function addRedactionValue(values, value, minLength) {
...
// src/runner.ts
async function runConfig(config, rootDir) {
  const results = [];
...
async function initConfig(options) {
  ...
  const targetDir = path5.dirname(path5.resolve(target));
  const outputFile = path5.join(targetDir, "aici-example.output.json");
  const schemaFile = path5.join(targetDir, "aici-example.schema.json");
...
`;
  ...
  await writeFile2(target, sample, { flag: writeFlag });
  await writeFile2(outputFile, `${JSON.stringify({
    decision: "approved",
    reason: "The response follows the contract."
  }, null, 2)}
`, { flag: writeFlag });
  await writeFile2(schemaFile, `${JSON.stringify({
    type: "object",
...
`, { flag: writeFlag });
  ...
  console.log(`Created ${path5.relative(process.cwd(), outputFile) || outputFile}`);
  console.log(`Created ${path5.relative(process.cwd(), schemaFile) || schemaFile}`);
...
function printSummary(passed, results, reportDir) {
